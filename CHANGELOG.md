...
+ [优化] 附件文字聊天等待改为对齐生图轮询节奏：提交后初始等待、按间隔查会话、429/5xx 指数退避并支持更长会话找回窗口，降低 ChatGPT 网页端 Too many requests。
+ [修复] 附件文字聊天会透传 `response_format`，JSON 回写请求只接受可解析 JSON 的最终回复，避免把 ChatGPT 的过程说明误当成结果。
+ [修复] 附件文字流式接口等待期间输出空内容心跳，避免长思考/长附件任务被前端或网关误判为断开，同时不污染最终 JSON 回写内容。
