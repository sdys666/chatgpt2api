...
def stream_text_chat_completion(backend, messages: list[dict[str, Any]], model: str, attachments: list[dict[str, Any]] | None = None, thinking_effort: str = "", response_format: str = "") -> Iterator[dict[str, Any]]:
    completion_id = f"chatcmpl-{uuid.uuid4().hex}"
    created = int(time.time())
    sent_role = False
    request = ConversationRequest(model=model, messages=messages, attachments=attachments, thinking_effort=thinking_effort, response_format=response_format)
    if attachments:
        sent_role = True
...
def chat_response_format_from_body(body: dict[str, Any]) -> str:
...
def text_chat_parts(body: dict[str, Any]) -> tuple[str, list[dict[str, Any]], list[dict[str, Any]], str, str]:
    ...
    return model, messages, chat_attachments_from_body(body), chat_thinking_effort_from_body(body), chat_response_format_from_body(body)
...
def handle(body: dict[str, Any]) -> dict[str, Any] | Iterator[dict[str, Any]]:
    if body.get("stream"):
        ...
        return chat_completion_cache.get_or_compute_stream(
            key,
            lambda: stream_text_chat_completion(text_backend(), messages, model, attachments, thinking_effort, response_format),
        )
    if is_image_chat_request(body):
